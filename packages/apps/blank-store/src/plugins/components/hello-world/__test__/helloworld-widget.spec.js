...
import {act, render, waitFor, screen} from '@oracle-cx-commerce/test/component/custom-render';
...
test('<HelloWorld/> unit/widget level testing with @testing-library/react', async () => {
  ...
  const {container} = await act(() => render(<Widget widgetId="helloWorldWidget123" />, mockStore, comps));
